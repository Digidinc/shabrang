...
const ShabrangViral = {
    ...
    setupImageSharing() {
        // Add share overlay to all images in chapter content
        document.querySelectorAll('.chapter-content img').forEach(img => {
            this.makeImageShareable(img);
        });
    },

    makeImageShareable(img) {
        // Wrap image in a container if not already
        let container = img.parentElement;
        if (!container.classList.contains('shareable-img-container')) {
            container = document.createElement('div');
            container.className = 'shareable-img-container';
        ...
        overlay.className = 'img-share-overlay';
        overlay.innerHTML = `
            <button class="img-share-btn" title="Share this image">
        ...
        // Click handler
        overlay.querySelector('.img-share-btn').addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            this.shareImage(img);
        });
    ...
    shareImage(img) {
        ...
        const caption = img.nextElementSibling?.classList.contains('caption')
            ...
            : img.alt || 'Image from The Liquid Fortress';

        // Store image for sharing
        this.state.currentImage = img;
        this.state.imageCaption = caption;

        // Open image share modal
        this.openImageModal(img, caption);
    },

    openImageModal(img, caption) {
        // Create image share modal if it doesn't exist
        let modal = document.getElementById('image-share-modal');
        if (!modal) {
            modal = document.createElement('div');
            modal.id = 'image-share-modal';
            modal.className = 'share-modal hidden';
            modal.innerHTML = `
                <div class="share-modal-content">
                    <button class="share-close">&times;</button>
                    ...
                    <div class="image-preview-container">
                        <canvas id="image-share-canvas"></canvas>
                    </div>
                    <p class="image-caption-preview"></p>
                    <div class="platform-buttons">
                        <button class="platform-btn" data-platform="twitter" title="Share on X/Twitter">
                            <svg viewBox="0 0 24 24" width="24" height="24"><path fill="currentColor" d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                        </button>
                        ...
                        <button class="platform-btn" data-platform="copy-image" title="Copy Image">
                            <svg viewBox="0 0 24 24" width="24" height="24"><path fill="currentColor" d="M16 1H4c-1.1 0-2 .9-2 2v14h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h11c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 16H8V7h11v14z"/></svg>
                        </button>
                        <button class="platform-btn" data-platform="download-image" title="Download Image">
                            <svg viewBox="0 0 24 24" width="24" height="24"><path fill="currentColor" d="M19 9h-4V3H9v6H5l7 7 7-7zM5 18v2h14v-2H5z"/></svg>
                        </button>
                    </div>
                    <div class="referral-display">
            ...
            // Platform buttons
            modal.querySelectorAll('.platform-btn').forEach(btn => {
                btn.onclick = () => this.shareImageToPlatform(btn.dataset.platform);
            });
        }

        // Update modal content
        modal.querySelector('.image-caption-preview').textContent = caption;
        modal.querySelector('.referral-code').textContent = this.getReferralCode();

        // Generate image card with caption
        this.generateImageCard(img, caption);

        // Show modal
        modal.classList.remove('hidden');
    },

    generateImageCard(img, caption) {
        const canvas = document.getElementById('image-share-canvas');
        const ctx = canvas.getContext('2d');
        ...
        sourceImg.onload = () => {
            const padding = 40;
            const captionHeight = caption ? 120 : 60; // Space for caption + source
            const borderWidth = 8;
            ...
            canvas.height = imgHeight + (padding * 2) + captionHeight;
            ...
            // Inner teal border
            ctx.strokeStyle = '#4dc0b5';
            ctx.lineWidth = 2;
            ctx.strokeRect(20, 20, canvas.width - 40, canvas.height - 40);

            // Draw the image
            ctx.drawImage(sourceImg, padding, padding, imgWidth, imgHeight);

            // Caption area
            const captionY = padding + imgHeight + 30;
            ...
            if (caption) {
                ...
                // Draw caption lines (max 2)
                lines.slice(0, 2).forEach((l, i) => {
                    ctx.fillText(l, canvas.width / 2, captionY + (i * 30));
                });
            }
            ...
            ctx.fillText('shabrang.ca', canvas.width / 2, canvas.height - 25);
        };

        sourceImg.onerror = () => {
            // Fallback: just show the original image info
            console.log('Could not load image for canvas');
        };

        sourceImg.src = img.src;
    },

    shareImageToPlatform(platform) {
        const img = this.state.currentImage;
        const caption = this.state.imageCaption;
        const url = this.getShareUrl();
        const title = this.getChapterName();
        ...
        switch (platform) {
            case 'twitter':
                window.open(
                    `https://twitter.com/intent/tweet?text=${encodeURIComponent(`${caption} — ${title}`)}&url=${encodeURIComponent(url)}`,
                    '_blank',
                    'width=550,height=420'
                );
                break;
            ...
            case 'whatsapp':
                window.open(
                    `https://wa.me/?text=${encodeURIComponent(`${caption} — ${title}\n\n${url}`)}`,
                    '_blank'
                );
                break;
            ...
            case 'telegram':
                window.open(
                    `https://t.me/share/url?url=${encodeURIComponent(url)}&text=${encodeURIComponent(`${caption} — ${title}`)}`,
                    '_blank'
                );
                break;
            ...
            case 'download-image':
                this.downloadImage(img, caption);
                break;
        }
    ...
    downloadImage(img, caption) {
        // Use the canvas with caption
        const canvas = document.getElementById('image-share-canvas');
        const link = document.createElement('a');
        const filename = `shabrang-${caption.substring(0, 30).replace(/\s+/g, '-').toLowerCase()}.png`;
        link.download = filename;
        link.href = canvas.toDataURL('image/png');
        link.click();
        this.showToast('Image downloaded!');
