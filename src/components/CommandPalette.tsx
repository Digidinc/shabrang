        ...
        <div className="py-2 max-h-[60vh] overflow-y-auto bg-shabrang-parchment">
          {filteredItems.length === 0 ? (
            <div className="px-4 py-12 text-center">
              <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-shabrang-teal/10 mb-3">
                <svg className="w-6 h-6 text-shabrang-ink-dim" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                </svg>
              </div>
              <p className="text-sm text-shabrang-ink mb-1">
                {isFA ? `نتیجه‌ای یافت نشد` : `No results found`}
              </p>
              <p className="text-xs text-shabrang-ink-dim">
                {isFA ? `کلمه کلیدی دیگری را امتحان کنید` : `Try a different search term`}
              </p>
            </div>
          ) : (
